...
#Region "HSPI - Base Plugin API"
      ...
      Try
        ...
        Select Case dv_type
          Case "Modem Connection"
            ...
            If Regex.IsMatch(dv_addr, "-Conn(\d)$") = True Then
              lineNumber = Regex.Match(dv_addr, "-Conn(\d)$").Groups(1).Value
            End If
            ...
            If Not IsNothing(ModemInterface) = True Then
              Select Case dev_value
                Case -1
                  Call ModemInterface.InitHW()
                Case -2
                  Call ModemInterface.ShutdownHW()
              End Select
            Else
              '
          ...
          Case "Last Caller Attributes"
            ...
            Dim ModemInterface As hspi_modem = ModemInterfaces.Find(Function(ModemLine) ModemLine.lineNumber = lineNumber)
            If Not IsNothing(ModemInterface) = True Then
              Dim callerNumber As String = ModemInterface.CallerNumber()
              If callerNumber.Length > 0 Then

                Select Case dev_value
                  Case 0, 1, 2, 4, 8, 16, 32
                    Dim nmbr As String = Regex.Replace(callerNumber, "[^\d]", "")

                    Dim attrs As Integer = hspi_database.GetCallerAttributes(nmbr)
                    Dim attr As Integer = dev_value
                    If attr = 0 Then
                      attrs = attr
                    Else
                      attrs = attrs Or attr
                    ...
                    '
                    SetCallerAttributes(nmbr, attrs)

                    Dim CallerAttrs As String = FormatCallerAttrNames(attrs)
                    SetDeviceString(dv_addr, CallerAttrs)
                End Select

              Else
                '
                ' Update device string to indicate error
                '
                SetDeviceString(dv_addr, "Last Caller Not Found")
              End If
            Else
              '
              ' Write warning message
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      gNumberQueryURL = hs.GetINISetting("NmbrQuery", "URL", CID_QUERY_URL, gINIFile)
      gModemInterfaces = hs.GetINISetting("Interface", "Count", gModemInterfaces, gINIFile)
      ...
      If bDBInitialized = True Then
        ...
        If bVersionChanged Then
          ...
          '
          ' Update the Misc Device Settings
          '
          UpdateMiscDeviceSettings(True)
        End If

        '
        ' Check database tables
        '
        CheckDatabaseTable("tblCallerLog")
        CheckDatabaseTable("tblCallerDetails")
      ...
      For lineNumber As Integer = 1 To gModemInterfaces
        ...
        ' Initialze Modem
        '
        Dim ModemInterface As New hspi_modem(lineNumber)
        Dim bConnected As Boolean = ModemInterface.InitHW()
        ModemInterfaces.Add(ModemInterface)
      Next
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      '
      For Each modemInterface As hspi_modem In ModemInterfaces
        modemInterface.ShutdownHW()
      Next
...
#Region "HSPI - Public Sub/Functions"
  ...
  Public Function GetCallerSummary() As SortedList
    ...
    Return hspi_plugin.GetCallerSummary()
  ...
  Public Function BuildCallerLogSQL(ByVal dbTable As String, _
    ...
    Return hspi_plugin.BuildCallerLogSQL(dbTable, dbFields, startDate, endDate, filterType, sortOrder)
  ...
  Public Function QueryDatabase(ByVal strSQL As String) As DataSet
    ...
    Try
      ...
      If DBConnectionMain.State = ConnectionState.Open Then
        Return hspi_database.QueryDatabase(strSQL)
      Else
  ...
  Public Function ConvertDateTimeToEpoch(ByVal dateTime As DateTime) As Long
    ...
    Return hspi_database.ConvertDateTimeToEpoch(dateTime)
  ...
  Public Function ConvertEpochToDateTime(ByVal epochTicks As Long) As DateTime
    ...
    Return hspi_database.ConvertEpochToDateTime(epochTicks)
  ...
  Public Sub ResetStatistics()
    ...
    Try
      ...
      '
      For Each ModemInterface As hspi_modem In ModemInterfaces
        ModemInterface.CallCount = 0
        ModemInterface.RingCount = 0
        ModemInterface.DropCount = 0
      Next
...
#Region "HSPI - Public Properties"
  ...
  Public Function FormatCID(ByVal nmbr As String, ByVal name As String) As String
    ...
    Return hspi_plugin.FormatCID(nmbr, name)
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public ReadOnly Property GetModemStatus(ByVal lineNumber As Integer) As String
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public ReadOnly Property GetStatistics(ByVal lineNumber As Integer, ByVal StatisticsType As String) As Integer
    ...
    Get
      ...
      If Not IsNothing(ModemInterface) = True Then
        Select Case StatisticsType
          Case "CallCount"
            Return ModemInterface.CallCount
          Case "RingCount"
            Return ModemInterface.RingCount
          Case "DropCount"
            Return ModemInterface.DropCount
          Case Else
            Return 0
        End Select
      Else
